...
const continuousPrewarm = window.TakeoffContinuousPrewarm;
...
function syncContinuousPageFromView() {
  ...
  updatePageLabel(); updateScaleLabel(); renderList(); scheduleContinuousLayerBackfill([page], usesPdfDetailTile() ? pdfDetailTile.baseRenderScale(desiredPdfRenderScale()) : desiredPdfRenderScale());
}
...
const continuousPrewarmController = continuousPrewarm.createContinuousPrewarmController({ canPrewarm: eligibility => state.pdf && !state.continuousScrollMode && eligibility?.eligible, groupPages: continuousGroupPages, cachedMatches: cachedContinuousLayerMatches, samePageNumbers: continuousRenderer.samePageNumbers, prewarm: prewarmContinuousLayer }); let continuousLayerFillKey = 0;
function invalidateContinuousPrewarm() { continuousLayerFillKey += 1; continuousPrewarmController.invalidate(); state.cachedContinuousPageLayout = null; } function scheduleContinuousLayerPrewarm(eligibility = continuousEligibility()) { continuousPrewarmController.schedule(eligibility); }
async function prewarmContinuousLayer(pages, { isCurrent = () => true } = {}) { const stillCurrent = () => { const eligibility = continuousEligibility(); return isCurrent() && state.pdf && eligibility.eligible && continuousRenderer.samePageNumbers(continuousGroupPages(eligibility), pages); }; if (!stillCurrent()) return false; const baseScale = usesPdfDetailTile() ? pdfDetailTile.baseRenderScale(desiredPdfRenderScale()) : desiredPdfRenderScale(); const result = await continuousRenderer.renderContinuousPdf({ pageCount: state.pdfPages, pages, requestedScale: baseScale, maxBitmapEdge: state.maxPdfBitmapEdge, cacheGet, renderPage: (page, requestedScale) => renderPageToCanvas(page, requestedScale, { reason: 'continuous-prewarm' }), isCurrent: stillCurrent, canvas: baseCanvas, context: baseCtx, pageLayer: $('continuousBasePages'), activatePageLayer: false, configureCanvasCssSize }); if (!result || !stillCurrent()) return false; state.cachedContinuousPageLayout = result.layout; return true; }
function scheduleContinuousLayerBackfill(pages, requestedScale) { const key = continuousLayerFillKey, layer = $('continuousBasePages'), layout = state.cachedContinuousPageLayout; setTimeout(async () => { for (const page of pages) { const box = continuousRenderer.pageBoxForPage(layout, page), child = continuousRenderer.continuousLayerPageElement(layer, page); if (!box || child?.dataset?.rendered === 'true') continue; const entry = cacheGet(page, requestedScale) || await renderPageToCanvas(page, requestedScale, { reason: 'continuous-background' }); if (key !== continuousLayerFillKey || !state.pdf || !state.cachedContinuousPageLayout) return; continuousRenderer.replaceContinuousLayerPage({ layer, entry: { ...entry, page }, pageBox: box }); await new Promise(resolve => setTimeout(resolve, 0)); } }, 0); } async function continuousShellLayout(pages, entries) { const renderedPages = new Set(entries.map(entry => entry.page)); const pageInfos = await Promise.all(pages.filter(page => !renderedPages.has(page)).map(page => state.pdf.getPageInfo(page))); return continuousRenderer.buildContinuousShellLayout({ pages, entries, pageInfos }); }
const PALETTE = [
...
function updateEndpointSnapConnection(measurement, endpoint, snap) {
  if (!endpoint) return;
  if (snap?.kind === 'anchor' && snap.endpoint) measurementCommands.setEndpointSnapConnection(measurement, endpoint, snap);
  else measurementCommands.clearEndpointSnapConnection(measurement, endpoint);
}
...
async function renderContinuousPdfPage({ fit = true, resetInteraction = true, minRenderScale = desiredPdfRenderScale(), shouldApply = () => true, reason = 'continuous-render' } = {}) {
  ...
  if (cachedContinuousLayerMatches(pages, cachedLayer)) {
    ...
    scheduleContinuousLayerBackfill(pages, baseScale);
    return true;
  }
  continuousPrewarmController.invalidate(); continuousLayerFillKey += 1;
  state.navToken++; const token = state.navToken;
  const p = state.pdfPage, current = cacheGet(p, baseScale) || await renderPageToCanvas(p, baseScale, { reason });
  if (token !== state.navToken || !shouldApply()) return false;
  const entries = pages.map(page => { const entry = page === p ? current : cacheGet(page, baseScale); return entry ? { ...entry, page } : null; }).filter(Boolean);
  const layout = await continuousShellLayout(pages, entries);
  continuousRenderer.paintContinuousPageShells({ layer: cachedLayer, canvas: baseCanvas, context: baseCtx, entries, layout, activate: true });
  configureCanvasCssSize(baseCanvas, layout.width, layout.height);
  state.baseW = layout.width; state.baseH = layout.height; state.continuousPageLayout = layout;
  state.cachedContinuousPageLayout = layout;
  configureDrawCanvas(); onPageReady({ fit, resetInteraction });
  if (usesPdfDetailTile()) await pdfDetailTile.renderNow({ reason: `${reason}-detail` });
  scheduleContinuousLayerBackfill(pages, baseScale);
  return true;
...
stage.addEventListener('mousedown', (e) => {
  ...
  if (state.mode === 'selection') {
    ...
    const rawInfo = drawingPointInfo(p);
    if (!rawInfo) return;
    if (state.inProgress && rawInfo.page !== state.inProgress.page) { showStatus('Finish calibration on the same page.', 2200); redraw(); return; }
    const previous = state.inProgress?.points?.[state.inProgress.points.length - 1] || null;
    const drawInfo = placementPointInfo(p, { page: state.inProgress?.page || rawInfo.page, previous, shiftKey: e.shiftKey });
    setContinuousCurrentPage(drawInfo.page);
    if (!state.inProgress) {
      state.inProgress = { type: 'calib', page: drawInfo.page, points: [drawInfo.point] };
    } else {
      state.inProgress = pointerWorkflow.appendPointToDraft({ inProgress: state.inProgress, point: drawInfo.point });
      openCalibModal();
    }
    redraw();
    ...
    const previous = state.inProgress?.points?.[state.inProgress.points.length - 1] || null;
    const excludeMeasurementIds = state.inProgress?.continuation?.measurementId ? [state.inProgress.continuation.measurementId] : [];
...
stage.addEventListener('mousemove', (e) => {
  ...
  if (state.dragMeasurement) {
    ...
    if (m) {
      const result = pointerWorkflow.applyMeasurementDrag({
        measurement: m,
        ...
        snapDelta: ({ dx, dy, originalPoints, originalSegments }) => state.snapToPaths
...
function finishFreehandMeasurement() {
  ...
  applyDraftSnapConnections(measurement, draft);
  const result = measurementWorkflows.appendMeasurementResult({
    measurements: state.measurements,
    measurement,
    selectedId: state.selectedId,
...
function getEffectiveCursor() {
  ...
  const page = state.inProgress.page || currentPage();
  const points = state.inProgress.points || [];
  const previous = points.length ? points[points.length - 1] : null;
  const excludeMeasurementIds = state.inProgress.continuation?.measurementId ? [state.inProgress.continuation.measurementId] : [];
  const info = placementPointInfo(state.cursorImg, { page, previous, shiftKey: state.shiftHeld, excludeMeasurementIds });
  return info?.point || continuousMeasurements.localPointForPage(state, page, state.cursorImg);
}
