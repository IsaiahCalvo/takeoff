...
test('continuous scroll prioritizes the newly visible page render', async () => {
  const main = await readFile(new URL('../src/main.js', import.meta.url), 'utf8');
  const syncFromView = main.match(/function syncContinuousPageFromView\(\) \{[\s\S]*?\n\}/)?.[0] || '';

  assert.match(syncFromView, /scheduleContinuousLayerBackfill\(\[page\]/);
});
...
test('continuous page prewarm is deferred off upload paint and consumed by toggle', async () => {
  ...
  assert.match(renderContinuous, /paintContinuousPageShells/);
  assert.match(renderContinuous, /scheduleContinuousLayerBackfill\(pages,\s*baseScale\)/);
  assert.match(toggleHandler, /state\.continuousScrollAutoEnable = false/);
  assert.doesNotMatch(prewarm, /Promise\.resolve\(\)\.then/);
});

test('new PDFs render the first visible state through continuous mode', async () => {
  const main = await readFile(new URL('../src/main.js', import.meta.url), 'utf8');
  ...
  const loadFile = main.match(/async function loadFile[\s\S]*?\nfunction resetDocState/)?.[0] || '';
...
test('All Pages tab renders page dropdown sections with individual runs', async () => {
  ...
  const pageCopyRule = styles.match(/\.page-group-copy\s*\{[^}]+\}/)?.[0] || '';
  const pageMetaRule = styles.match(/\.path-group-meta\.page-group-meta\s*\{[^}]+\}/)?.[0] || '';
  const pageSummaryRule = styles.match(/\.page-group-summary\s*\{[^}]+\}/)?.[0] || '';
  const pageAverageRule = styles.match(/\.page-group-average\s*\{[^}]+\}/)?.[0] || '';
  ...
  assert.match(pageCopyRule, /justify-items:\s*start/);
  assert.match(pageMetaRule, /display:\s*block/);
  assert.match(pageMetaRule, /text-align:\s*left/);
  assert.match(pageSummaryRule, /justify-items:\s*end/);
  assert.match(pageSummaryRule, /text-align:\s*right/);
  assert.match(pageAverageRule, /text-align:\s*right/);
...
test('Path group rows nest full-width child runs with existing row controls', async () => {
  ...
  assert.match(childRowRule, /grid-template-columns:\s*18px minmax\(0,\s*1fr\) minmax\(20px,\s*auto\) minmax\(46px,\s*auto\) 18px 18px/);
...
test('hidden category styling removes the icon shell and uses an icon-only bulb state', async () => {
  ...
  const headerSubReservedRule = styles.match(/\.side-h:has\(\.entire-total:not\(\[hidden\]\)\) \.sub\s*\{[^}]+\}/)?.[0] || '';
  ...
  const categoryRowRule = styles.match(/\.path-category-row\s*\{[^}]+\}/)?.[0] || '';
  const categoryMetaRule = styles.match(/\.path-category-meta\s*\{[^}]+\}/)?.[0] || '';
  const categoryControlsRule = styles.match(/\.path-category-controls\s*\{[^}]+\}/)?.[0] || '';
  const categoryHiddenTotalRule = styles.match(/\.path-category-hidden-total\s*\{[^}]+\}/)?.[0] || '';
  const statusRule = styles.match(/\.path-category-status\s*\{[^}]+\}/)?.[0] || '';
  const rowStatusRule = styles.match(/\.path-category-row \.path-category-status\s*\{[^}]+\}/)?.[0] || '';
  ...
  assert.match(headerTotalRule, /bottom:\s*32px/);
  assert.match(headerTotalRule, /text-align:\s*right/);
  assert.match(headerSubRule, /padding-right:\s*0/);
  assert.match(headerSubReservedRule, /padding-right:\s*0/);
  ...
  assert.match(categoryRowRule, /height:\s*44px/);
  assert.match(categoryMetaRule, /display:\s*grid/);
  assert.match(categoryMetaRule, /gap:\s*2px/);
  assert.doesNotMatch(categoryMetaRule, /flex-wrap/);
  assert.match(categoryControlsRule, /position:\s*relative/);
  assert.match(categoryControlsRule, /width:\s*74px/);
  assert.match(categoryControlsRule, /height:\s*30px/);
  assert.match(categoryHiddenTotalRule, /position:\s*absolute/);
  assert.match(categoryHiddenTotalRule, /right:\s*0/);
  assert.match(categoryHiddenTotalRule, /bottom:\s*0/);
  assert.match(statusRule, /width:\s*24px/);
  assert.match(statusRule, /height:\s*24px/);
  assert.match(rowStatusRule, /width:\s*18px/);
  assert.match(rowStatusRule, /height:\s*18px/);
