...
test('continuous zoom sharpening redraws the PDF stack when detail tiles are unavailable', async () => {
  ...
  assert.deepEqual(Array.from(state.preRenderQueue), []);
  assert.equal(detailRenders.length, 0);
  assert.equal(fullStackRenders.length, 1);
  assert.equal(fullStackRenders[0].reason, 'zoom-sharpen');
  assert.equal(fullStackRenders[0].minRenderScale, 12);
});
