...
async function loadInputController() {
  const source = await readFile(new URL('../src/app/input-controller.js', import.meta.url), 'utf8');
...
test('describeKeyDown maps mode hotkeys and skips text targets', async () => {
  ...
  assert.deepEqual(plain(input.describeKeyDown({ key: 'e' }, {})), { action: 'set-draw-tool', tool: 'circle' });
  assert.deepEqual(plain(input.describeKeyDown({ key: 'A' }, {})), { action: 'set-draw-tool', tool: 'arc' });
